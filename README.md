...
│   └── 17_write_manuscript.py                      # Full manuscript .docx
├── figures/
...
python scripts/17_write_manuscript.py
```

## Manuscript Draft

`paper/Bangladesh_HPAI_Manuscript.docx` is a full IMRaD draft (Abstract,
Introduction, Methods, Results with 3 in-text tables, Discussion,
Conclusions), generated by `scripts/17_write_manuscript.py`. Every number
in it is pulled directly from this repo's own output tables — nothing is
invented. Figures are placed inline, immediately after the paragraph that
discusses each one (not batched at the end) — note Figure 3 (the division
choropleth map) appears before Figure 4 (species) in the document because
it's discussed alongside Figure 2 in Section 3.2, so figure numbers don't
